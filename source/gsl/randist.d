...
extern (C)
{
  ...
  void gsl_ran_bivariate_gaussian(const gsl_rng* r, double sigma_x,
    double sigma_y, double rho, double* x, double* y);
  double gsl_ran_bivariate_gaussian_pdf(const double x, const double y,
  ...
  void gsl_ran_multinomial(const gsl_rng* r, const size_t K, const uint N, const double[] p,
    uint[] n);
  ...
  void gsl_ran_sample(const gsl_rng* r, void* dest, size_t k, void* src, size_t n,
    size_t size);
...
@Name("test_random_number_generation")
unittest
{
  ...
  gsl_rng_set(r, 10);
  auto x = gsl_ran_gaussian(r, 1);
  assert(approxEqual(x, -0.746100));
}
