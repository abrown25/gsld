...
extern (C)
{
  ...
  gsl_matrix* gsl_matrix_alloc_from_block(gsl_block* b, const size_t offset,
    const size_t n1, const size_t n2, const size_t d2);

  gsl_matrix* gsl_matrix_alloc_from_matrix(gsl_matrix* m, const size_t k1,
    const size_t k2, const size_t n1, const size_t n2);
  ...
  _gsl_matrix_view gsl_matrix_submatrix(gsl_matrix* m, const size_t i,
    const size_t j, const size_t n1, const size_t n2);
  ...
  _gsl_vector_view gsl_matrix_subrow(gsl_matrix* m, const size_t i, const size_t offset,
    const size_t n);

  _gsl_vector_view gsl_matrix_subcolumn(gsl_matrix* m, const size_t j,
    const size_t offset, const size_t n);

  _gsl_matrix_view gsl_matrix_view_array(double* base, const size_t n1, const size_t n2);

  _gsl_matrix_view gsl_matrix_view_array_with_tda(double* base, const size_t n1,
    const size_t n2, const size_t tda);

  _gsl_matrix_view gsl_matrix_view_vector(gsl_vector* v, const size_t n1, const size_t n2);

  _gsl_matrix_view gsl_matrix_view_vector_with_tda(gsl_vector* v,
    const size_t n1, const size_t n2, const size_t tda);

  _gsl_matrix_const_view gsl_matrix_const_submatrix(const gsl_matrix* m,
    const size_t i, const size_t j, const size_t n1, const size_t n2);
  ...
  _gsl_vector_const_view gsl_matrix_const_subrow(const gsl_matrix* m,
    const size_t i, const size_t offset, const size_t n);

  _gsl_vector_const_view gsl_matrix_const_subcolumn(const gsl_matrix* m,
    const size_t j, const size_t offset, const size_t n);

  _gsl_matrix_const_view gsl_matrix_const_view_array(const double* base,
    const size_t n1, const size_t n2);

  _gsl_matrix_const_view gsl_matrix_const_view_array_with_tda(const double* base,
    const size_t n1, const size_t n2, const size_t tda);

  _gsl_matrix_const_view gsl_matrix_const_view_vector(const gsl_vector* v,
    const size_t n1, const size_t n2);

  _gsl_matrix_const_view gsl_matrix_const_view_vector_with_tda(const gsl_vector* v,
    const size_t n1, const size_t n2, const size_t tda);
  ...
  void gsl_matrix_minmax_index(const gsl_matrix* m, size_t* imin, size_t* jmin,
  ...
  double* gsl_matrix_const_ptr(const gsl_matrix* m, const size_t i, const size_t j);
  ...
  version (HAVE_INLINE)
  {
    ...
    double* gsl_matrix_ptr(gsl_matrix* m, const size_t i, const size_t j)
    {
      return cast(double*)(m.data + (i * m.tda + j));
    }

    const double* gsl_matrix_const_ptr(const gsl_matrix* m, const size_t i, const size_t j)
    {
      return cast(const(double*))(m.data + (i * m.tda + j));
    }
  }
}

gsl_matrix* d_gsl_matrix_double_set(double[] vector, size_t nRow)
{
  size_t nCol = vector.length / nRow;
  gsl_matrix* matrix = gsl_matrix_alloc(nRow, nCol);

  foreach (i; 0 .. nRow)
  {
    foreach (j; 0 .. nCol)
    {
      gsl_matrix_set(matrix, i, j, vector[i + j * nRow]);
    }
  }
  return matrix;
}
...
unittest
{
  ...
  gsl_matrix* matrix = d_gsl_matrix_double_set(x, nRow);
  assert(gsl_matrix_get(matrix, 1, 0) == 2);
  assert(gsl_matrix_get(matrix, 1, 2) == 8);
  assert(gsl_matrix_get(matrix, 1, 1) == 5);
  assert(gsl_matrix_get(matrix, 0, 1) == 9);
}
